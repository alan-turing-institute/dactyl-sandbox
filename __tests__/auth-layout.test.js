...
describe('minimal auth screen layout', () => {
  test('auth markup keeps context, credentials, and actions in separate groups', () => {
    ...
    expect(indexHtml).toContain('id="forgot-password-button" class="link-action" type="button"');
    expect(indexHtml).toContain('New here?');
  });

  test('visible labels remain available and redundant validation help is visually hidden', () => {
    ...
    expect(indexHtml).toContain('id="auth-help" class="auth-help sr-only"');
  ...
  test('CSS makes login primary, signup secondary, and tap targets mobile-safe', () => {
    ...
    expect(styles).toContain('.auth-forgot-action {');
